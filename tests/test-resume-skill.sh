...
grep -Fq '\begin{minipage}[c]{0.13\textwidth}' "$template/resume.cls"
grep -Fq '\begin{minipage}[c]{0.74\textwidth}' "$template/resume.cls"
grep -Fq '\newcommand{\ResumeCompactSections}' "$template/resume.cls"
grep -Fq '\datedsubsection{\textbf{#1}}{#3}' "$template/resume.cls"
grep -Fq '\schoolbadge{0.092}' "$template/resume.tex"
grep -Fq '\ResumeSection{\faGraduationCap}{教育背景}' "$template/resume.tex"
grep -Fq '\ResumeCompactSections' "$template/resume.tex"
...
grep -Fq '这些是版式契约，不是候选人内容约束' "$root/references/latex-delivery.md"
