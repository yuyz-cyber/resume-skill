...
- **成熟 LaTeX 母版**：内置可直接编译的一页中文简历母版，提供稳定的图标、字号、项目层级、关键内容高亮及校徽与证件照占位资源。
...
└── assets/
    └── latex-template/
        ...
        ├── fontawesome.sty          # 自包含的章节与联系方式图标
        ├── fontawesomesymbols-*.tex # FontAwesome 图标映射
        ├── fonts/                   # 中文、西文及图标字体与许可证
        └── images/
            ├── school.png           # 校徽占位图
            └── you.png              # 中性证件照占位图
```

`SKILL.md` 只负责串联通用流程；`references/` 按核心特性提供专项规则；`scripts/` 执行确定性检查；`assets/` 提供完整的一页 LaTeX 视觉母版。
